...
def performDBA(series, n_iterations=10, pbar=None):
    n_series = len(series)
    max_length = 0
    for s in series:
    ...
    medoid_ind = approximate_medoid_index(
        series, cost_mat, delta_mat, tmp_delta_mat, pbar)
    center = series[medoid_ind]

    for i in range(0, n_iterations):
        center = DBA_update(center, series, cost_mat,
                            path_mat, delta_mat, tmp_delta_mat)

    return center


def approximate_medoid_index(series, cost_mat, delta_mat, tmp_delta_mat, pbar=None):
    if len(series) <= 50:
    ...
    for index_candidate in indices:
        ...
        ss = sum_of_squares(candidate, series, cost_mat,
                            delta_mat, tmp_delta_mat)
...
def DTW(s, t, cost_mat=None, delta_mat=None, tmp_delta_mat=None):
    max_length = 0
    for serie in [s, t]:
        max_length = max(max_length, serie.shape[1])

    if cost_mat is None:
        cost_mat = np.zeros((max_length, max_length))
    if delta_mat is None:
        delta_mat = np.zeros((max_length, max_length))
    if tmp_delta_mat is None:
        tmp_delta_mat = np.zeros((max_length, max_length))
...
def squared_DTW(s, t, cost_mat, delta_mat, tmp_delta_mat):
    ...
    for i in range(1, s_len):
        for j in range(1, t_len):
            diag, left, top = cost_mat[i-1, j -
                                       1], cost_mat[i, j-1], cost_mat[i-1, j]
            if (diag <= left):
                if (diag <= top):
                    res = diag
                else:
...
def fill_delta_mat_dtw(center, s, delta_mat, tmp_delta_mat):
    ...
    assert (
        np.abs(np.sum(np.square(center[:, 0]-s[:, 0]))-delta_mat[0, 0]) <= 1e-6)
